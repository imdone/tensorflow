...
class SDCALogisticClassifierTest(test.TestCase):
  ...
  def _single_threaded_test_session(self):
    # TODO (andreasst): figure out why SDCALinearRegressor needs a single id:1605
    # https://github.com/imdone/tensorflow/issues/1605
...
class SDCALinearRegressorTest(test.TestCase):
  ...
  def _single_threaded_test_session(self):
    # TODO (andreasst): figure out why SDCALinearRegressor needs a single id:1183
    # https://github.com/imdone/tensorflow/issues/1184
    # threaded session to pass in tsan mode but SDCALogisticClassifier does not.
    config = config_pb2.ConfigProto(
        inter_op_parallelism_threads=1, intra_op_parallelism_threads=1)
    return self.test_session(config=config)
  ...
  def testBiasAndOtherColumns(self):
    ...
    with self._single_threaded_test_session():
      ...
      # TODO (b/29339026): Change the expected results to expect a centered bias. id:1066
      # https://github.com/imdone/tensorflow/issues/1067
      self.assertNear(
