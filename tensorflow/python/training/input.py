...
def slice_input_producer(tensor_list, num_epochs=None, shuffle=True, seed=None,
  ...
  with ops.name_scope(name, "input_producer", tensor_list):
    ...
    # TODO (josh11b): Add an assertion that the first dimension of id:4328
    # https://github.com/imdone/tensorflow/issues/4326
...
def _batch(tensors, batch_size, keep_input, num_threads=1, capacity=32,
  ...
  with ops.name_scope(name, "batch", list(tensor_list) + [keep_input]) as name:
    ...
    # TODO (josh11b,mrry): Switch to BatchQueue once it is written. id:4008
    # https://github.com/imdone/tensorflow/issues/4006
...
# TODO (josh11b): Add a thread_multiplier or num_threads (that has to be id:3696
# https://github.com/imdone/tensorflow/issues/3695
...
def _batch_join(tensors_list, batch_size, keep_input, capacity=32,
  ...
  with ops.name_scope(name, "batch_join",
    ...
    # TODO (josh11b,mrry): Switch to BatchQueue once it is written. id:3473
    # https://github.com/imdone/tensorflow/issues/3472
    queue = _which_queue(dynamic_pad)(
        capacity=capacity, dtypes=types, shapes=shapes, shared_name=shared_name)
