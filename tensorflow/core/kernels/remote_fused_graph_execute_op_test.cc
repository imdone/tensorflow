...
TEST_F(RemoteFusedGraphExecuteTest, BuildModelWithOneDataType) {
  ...
  TF_ASSERT_OK(
      NodeDefBuilder("remote_fused_graph_execute_op", "RemoteFusedGraphExecute")
          ...
          .Finalize(node_def()));
  TF_ASSERT_OK(InitOp());
  // TODO (satok): Add benchmark id:3304
  // https://github.com/imdone/tensorflow/issues/3303
}
...
TEST_F(RemoteFusedGraphExecuteTest, BuildModelWithWrongDataType) {
  ...
  ASSERT_FALSE(
      NodeDefBuilder("remote_fused_graph_execute_op", "RemoteFusedGraphExecute")
          ...
          .Finalize(node_def())
          .ok());
  // TODO (satok): Add benchmark id:3970
  // https://github.com/imdone/tensorflow/issues/3968
}
...
static RemoteFusedGraphExecuteInfo BuildRemoteFusedGraphExecuteInfo(
  ...
  for (const NodeDef& node : original_graph.node()) {
    ...
    // TODO (satok): Use TensorShapeMap to detime tensor shape type id:2254
    // https://github.com/imdone/tensorflow/issues/2253
...
class SampleRemoteFusedGraphExecutor final : public IRemoteFusedGraphExecutor {
 public:
  ...
  bool ExecuteGraph() final {
    CHECK(info_ != nullptr);
    // TODO (satok): Add utilities to implement this function more easily. id:2171
    // https://github.com/imdone/tensorflow/issues/2170
    ...
    // TODO (satok): Read NAME_B from node_a_plus_b id:2696
    // https://github.com/imdone/tensorflow/issues/2695
  ...
  bool ReadOutputNode(const string& node_name,
    ...
    // TODO (satok): Specify tensor shape by using default_graph_tensor_shape. id:3307
    // https://github.com/imdone/tensorflow/issues/3306
