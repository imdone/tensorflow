...
void PriorityQueue::TryDequeue(OpKernelContext* ctx,
  ...
  bool already_cancelled;
  {
    ...
    if (!already_cancelled) {
      // TODO (josh11b): This makes two copies of callback, avoid this if possible. id:2233
      // https://github.com/imdone/tensorflow/issues/2232
...
void PriorityQueue::TryDequeueMany(int num_elements, OpKernelContext* ctx,
  ...
  if (num_elements == 0) {
    ...
    tuple.reserve(num_components());
    for (int i = 0; i < num_components(); ++i) {
      // TODO (josh11b,misard): Switch to allocate_output(). Problem is id:2148
      // https://github.com/imdone/tensorflow/issues/2147
  ...
  bool already_cancelled;
  {
    ...
    already_cancelled = !cm->RegisterCallback(
        token, [this, cm, token]() { Cancel(kDequeue, cm, token); });
    if (!already_cancelled) {
      // TODO (josh11b): This makes two copies of callback, avoid this if possible. id:2567
      // https://github.com/imdone/tensorflow/issues/2566
      dequeue_attempts_.emplace_back(
          ...
          [callback, this,
           allow_small_batch](Attempt* attempt) EXCLUSIVE_LOCKS_REQUIRED(mu_) {
            ...
            // TODO (ebrevdo): Change approach if this leads to locking issues. id:3270
            // https://github.com/imdone/tensorflow/issues/3269
