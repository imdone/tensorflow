...
void RandomShuffleQueue::TryDequeue(OpKernelContext* ctx,
  ...
  bool already_cancelled;
  {
    ...
    if (!already_cancelled) {
      // TODO (josh11b): This makes two copies of callback, avoid this if possible. id:2159
      // https://github.com/imdone/tensorflow/issues/2158
...
void RandomShuffleQueue::TryDequeueMany(int num_elements, OpKernelContext* ctx,
  ...
  if (num_elements == 0) {
    ...
    tuple.reserve(num_components());
    for (int i = 0; i < num_components(); ++i) {
      // TODO (josh11b,misard): Switch to allocate_output(). Problem is id:2581
      // https://github.com/imdone/tensorflow/issues/2580
  ...
  bool already_cancelled;
  {
    ...
    already_cancelled = !cm->RegisterCallback(
        token, [this, cm, token]() { Cancel(kDequeue, cm, token); });
    if (!already_cancelled) {
      // TODO (josh11b): This makes two copies of callback, avoid this if possible. id:3281
      // https://github.com/imdone/tensorflow/issues/3280
      dequeue_attempts_.emplace_back(
